...
export default function ApplicationConfigAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('applicationConfig.add.headline'));
  ...
  const createApplicationConfig = async (data: ApplicationConfigDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/applicationConfigs', data);
      navigate('/applicationConfigs', {
            state: {
              msgSuccess: t('applicationConfig.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('applicationConfig.add.headline')}</h1>
      <div>
        <Link to="/applicationConfigs" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('applicationConfig.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createApplicationConfig)} noValidate>
      ...
      <input type="submit" value={t('applicationConfig.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
