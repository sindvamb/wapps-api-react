...
export default function EmployeeAdd() {
  ...
  useDocumentTitle(t('employee.add.headline'));
  ...
  const createEmployee = async (data: EmployeeDTO) => {
    ...
    try {
      ...
      navigate('/employees', {
            state: {
              msgSuccess: t('employee.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('employee.add.headline')}</h1>
      <div>
        <Link to="/employees" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('employee.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createEmployee)} noValidate>
      ...
      <input type="submit" value={t('employee.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
