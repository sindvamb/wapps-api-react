...
export default function PartnerUnitEdit() {
  const { t } = useTranslation();
  useDocumentTitle(t('partnerUnit.edit.headline'));
  ...
  const prepareForm = async () => {
    try {
      ...
      const data = (await axios.get('/api/partnerUnits/' + currentId)).data;
  ...
  const updatePartnerUnit = async (data: PartnerUnitDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.put('/api/partnerUnits/' + currentId, data);
      navigate('/partnerUnits', {
            state: {
              msgSuccess: t('partnerUnit.update.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('partnerUnit.edit.headline')}</h1>
      <div>
        <Link to="/partnerUnits" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('partnerUnit.edit.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(updatePartnerUnit)} noValidate>
      <input type="submit" value={t('partnerUnit.edit.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6 mb-5" />
      <InputRow useFormResult={useFormResult} object="partnerUnit" field="id" disabled={true} />
      ...
      <input type="submit" value={t('partnerUnit.edit.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
